...
import { Settings, Zap, Calendar as CalendarIcon, TrendingUp } from 'lucide-react';
...
export default function Dashboard({ targets, history, onUpdateDay, onNavigate }) {
  ...
  const handleToggleHabit = (id) => {
    ...
    const isCompleted = todayData[id] === (HABITS.find(h => h.id === id).isBoolean ? true : targets[id]);
    const nextValue = isCompleted ? 0 : (HABITS.find(h => h.id === id).isBoolean ? true : targets[id]);
  ...
  const getTip = () => {
    ...
    last7Days.forEach(date => {
      ...
      HABITS.forEach(h => {
        if (data[h.id] !== undefined) {
          scores[h.id].count += 1;
          const val = data[h.id];
          const isMet = h.isBoolean ? val === targets[h.id] : val >= targets[h.id];
    ...
    HABITS.forEach(h => {
      const s = scores[h.id].count ? (scores[h.id].total / scores[h.id].count) : 0;
      if (s < lowestScore) {
        lowestScore = s;
        lowestHabit = h;
      }
    ...
    const tips = {
      ...
      sleep: "Your sleep average is low this week. Try winding down 30 mins earlier tonight!",
      activity: "Remember to stay active! A simple 15-min walk makes a big difference.",
      ...
      stress_relief: "Take a deep breath. 5 minutes of meditation or stretching could help right now!"
    };
    ...
    for (let i = 0; i < 14; i++) {
      ...
      HABITS.forEach(h => {
        ...
        if (h.isBoolean) {
          ...
          mockResult[h.id] = rand > 0.4 ? targets[h.id] : Math.floor(targets[h.id] / 2); // 60% chance of full, 40% half
        }
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-12 mb-20">
      ...
      >
        <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tight">Today</h1>
      ...
      >
        {HABITS.map((habit) => {
          const isCompleted = todayData[habit.id] === (habit.isBoolean ? true : targets[habit.id]);
          return (
            <HabitCard 
              key={habit.id}
              habit={habit}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          ...
          <StreakCalendar history={history} targets={targets} calculateScore={calculateScore} />
        </motion.div>
        ...
        >
          ...
          <TrendChart history={history} targets={targets} calculateScore={calculateScore} />
        </motion.div>
      </div>
      ...
      >
        <h2 className="text-2xl font-black uppercase mb-4 text-neoYellow">AI-Free Tip Engine</h2>
